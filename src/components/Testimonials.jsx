...
import clientImage from "../assets/testImg1.jpg";
...
const Testimonials = () => { 
  ...
  const scrollRight = () => {
    const newIndex = Math.min(testimonials.length - 3, currentIndex + 1);
    setCurrentIndex(newIndex);
    scrollToTestimonial(newIndex);
  };
  ...
  if (error) return <div className="text-center py-12 text-red-500"></div>;
  if (!testimonials.length) return <div className="text-center py-4">No testimonials available</div>;
    ...
    <div className='max-w-7xl mx-auto px-4 pt-12 pb-16 relative'>
      ...
      <div className='relative'>
        ...
        <button 
          ...
          className={`absolute -right-4 top-1/2 transform -translate-y-1/2 z-10 bg-white p-3 rounded-full shadow-md hover:bg-gray-100 transition-colors ${currentIndex >= testimonials.length - 3 ? 'opacity-50 cursor-not-allowed' : ''}`}
